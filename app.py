...
mydb= connection.MySQLConnection(user='root', password='root123',host='localhost',database='snmprg')
app=Flask(__name__) 
...
def addnotes():
    if session.get('users'):
        if request.method=='POST':
            ...
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select user_id from users where useremail=%s',[session.get('users')])
            uid=cursor.fetchone()
            if uid:
                try:
...
def viewallnotes():
    if session.get('users'):
        try:
            ...
            cursor.execute('select user_id from users where useremail=%s',[session.get('users')])
            uid=cursor.fetchone()
            cursor.execute('select n_id,title,description,created_at from notes where user_id=%s',[uid[0]])
    ...
    if session.get('users'):
        if request.method=='POST':
            ...
            if (pattern.match(search)):
                ...
                cursor.execute('select user_id from users where useremail=%s',[session.get('users')])
                uid=cursor.fetchone()
                cursor.execute('select * from notes where (n_id like %s or title like %s or description like %s or created_at like %s) and user_id=%s',[search+'%',search+'%',search+'%',search+'%',uid[0]])
